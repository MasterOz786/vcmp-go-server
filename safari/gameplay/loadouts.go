...
func clampPack(pack int) int {
	...
	if pack > apidef.MaxPack {
		return apidef.MaxPack
	}
	return pack
}

func LoadoutComplete(api apidef.API, playerID, team, pack int) bool {
	pack = clampPack(pack)
...
func ApplyLoadout(api apidef.API, playerID, team, pack int) {
	pack = clampPack(pack)
	var packs map[int]PackDef
	if team == apidef.TeamEscort {
		packs = EscortPacks()
...
func EnforceAllowed(api apidef.API, playerID, team, pack int) {
